...
![18333fig0501](figures/18333fig0501-tn.png){:caption="集中式工作流"}
...
![18333fig0502](figures/18333fig0502-tn.png){:caption="集成管理员工作流"}
...
![18333fig0503](figures/18333fig0503-tn.png){:caption="司令官与副官工作流"}

这种工作流程并不常用，只有当项目极为庞杂，或者需要多级别管理时，才会体现出优势。利用这种方式，项目总负责人（即司令官）可以把大量分散的集成工作委托给不同的小组负责人分别处理，最后再统筹起来，如此各人的职责清晰明确，也不易出错[^5-t1]。
...
如果你的提交说明都用这样的格式来书写，好多事情就可以变得十分简单。Git 项目本身就是这样要求的，我强烈建议你到 Git 项目仓库下运行 `git log --no-merges` 看看，所有提交历史的说明是怎样撰写的。[^5-t2]
...
![18333fig0504](figures/18333fig0504-tn.png){:caption="John 的仓库历史"}
...
![18333fig0505](figures/18333fig0505-tn.png){:caption="合并 origin/master 后 John 的仓库历史"}
...
![18333fig0506](figures/18333fig0506-tn.png){:caption="推送后 John 的仓库历史"}

而在这段时间，Jessica 已经开始在另一个特性分支工作了。她创建了 `issue54` 并提交了三次更新。她还没有下载 John 提交的合并结果，所以提交历史如图 5-7 所示：

![18333fig0507](figures/18333fig0507-tn.png){:caption="Jessica 的提交历史"}
...
![18333fig0508](figures/18333fig0508-tn.png){:caption="获取 John 的更新之后 Jessica 的提交历史"}
...
要合并 `origin/master` 或 `issue54` 分支，谁先谁后都没有关系，因为它们都在上游（upstream）[^5-t3]，所以无所谓先后顺序，最终合并后的内容快照都是一样的，而仅是提交历史看起来会有些先后差别。Jessica 选择先合并 `issue54`：
...
![18333fig0509](figures/18333fig0509-tn.png){:caption="合并 John 的更新后 Jessica 的提交历史"}
...
![18333fig0510](figures/18333fig0510-tn.png){:caption="Jessica 推送数据后的提交历史"}

以上就是最简单的协作方式之一：先在自己的特性分支中工作一段时间，完成后合并到自己的 `master` 分支；然后下载合并 `origin/master` 上的更新（如果有的话），再推回远程服务器。一般的协作流程如图 5-11 所示：

![18333fig0511](figures/18333fig0511-tn.png){:caption="多用户共享仓库协作方式的一般工作流程时序"}
...
![18333fig0512](figures/18333fig0512-tn.png){:caption="Jessica 的更新历史"}
...
![18333fig0513](figures/18333fig0513-tn.png){:caption="在特性分支中提交更新后的提交历史"}
...
![18333fig0514](figures/18333fig0514-tn.png){:caption="合并特性分支后的 Jessica 提交历史"}

许多开发小组改用 Git 就是因为它允许多个小组间并行工作，而在稍后恰当时机再行合并。通过共享远程分支的方式，无需干扰整体项目代码便可以开展工作，因此使用 Git 的小型团队间协作可以变得非常灵活自由。以上工作流程的时序如图 5-15 所示：

![18333fig0515](figures/18333fig0515-tn.png){:caption="团队间协作工作流程基本时序"}
...
在完成了特性分支开发，提交给项目维护者之前，先到原始项目的页面上点击“Fork”按钮，创建一个自己可写的公共仓库[^5-t4]。然后将此仓库添加为本地的第二个远端仓库，姑且称为 `myfork`：
...
`request-pull` 命令接受两个参数，第一个是本地特性分支开始前的原始分支，第二个是请求对方来抓取的 Git 仓库 URL[^5-t5]。比如现在Jessica 准备要给 John 发一个 pull requst，她之前在自己的特性分支上提交了两次更新，并把分支整个推到了服务器上，所以运行该命令会看到：
...
![18333fig0516](figures/18333fig0516-tn.png){:caption="featureB 以后的提交历史"}
...
![18333fig0517](figures/18333fig0517-tn.png){:caption="featureA 重新衍合后的提交历史"}

注意，此时推送分支必须使用 `-f` 选项[^5-t6]替换远程已有的 `featureA` 分支，因为新的 commit 并非原来的后续更新。当然你也可以直接推送到另一个新的分支上去，比如称作 `featureAv2`。
...
![18333fig0518](figures/18333fig0518-tn.png){:caption="featureBv2 之后的提交历史"}
...
如此一番后，有了两个提交要发到邮件列表。我们可以用 `git format-patch` 命令来生成 mbox 格式的文件然后作为附件发送。每个提交都会封装为一个 `.patch` 后缀的 mbox 文件，但其中只包含一封邮件，邮件标题就是提交消息[^5-t7]，邮件内容包含补丁正文和 Git 版本号。这种方式的妙处在于接受补丁时仍可保留原来的提交消息，请看接下来的例子：
...
如果你的 IMAP 服务器没有启用 SSL，就无需配置最后那两行，并且 host 应该以 `imap://` 开头而不再是有 `s` 的 `imaps://`。保存配置文件后，就能用 `git send-email` 命令把补丁作为邮件依次发送到指定的 IMAP 服务器上的文件夹中[^5-t8]：
...
![18333fig0519](figures/18333fig0519-tn.png){:caption="多个特性分支"}

![18333fig0520](figures/18333fig0520-tn.png){:caption="多个特性分支"}
...
![18333fig0521](figures/18333fig0521-tn.png){:caption="特性分支合并前"}

![18333fig0522](figures/18333fig0522-tn.png){:caption="特性分支合并后"}

![18333fig0523](figures/18333fig0523-tn.png){:caption="特性分支发布后"}
...
![18333fig0524](figures/18333fig0524-tn.png){:caption="管理复杂的并行贡献"}
...
![18333fig0525](figures/18333fig0525-tn.png){:caption="将特性并入长期分支"}
...
![18333fig0526](figures/18333fig0526-tn.png){:caption="挑拣（cherry-pick）之前的历史"}
...
![18333fig0527](figures/18333fig0527-tn.png){:caption="挑拣（cherry-pick）之后的历史"}
...
[^5-t1]: 译注：此乃分而治之
[^5-t2]: 译注：如果现在还没有克隆 git 项目源代码，是时候 `git clone git://git.kernel.org/pub/scm/git/git.git` 了。
[^5-t3]: 译注：想像分叉的更新像是汇流成河的源头，所以上游 upstream 是指最新的提交
[^5-t4]: 译注：即下面的 url 部分，参照后续的例子，应该是 `git://githost/simplegit.git`
[^5-t5]: 译注：即下面 `myfork` 所指的，自己可写的公共仓库
[^5-t6]: 译注：表示 force，不作检查强制重写
[^5-t7]: 译注：额外有前缀，看例子
[^5-t8]: 译注：这里就是 Gmail 的 `[Gmail]/Drafts` 文件夹。但如果你的语言设置不是英文，此处的文件夹 Drafts 字样会变为对应的语言。
